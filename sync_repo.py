...
PIPELINE = [
    ...
    # ⛔ ТРИ СИРОТЫ 24.08.2026 — ИМЕННО ИЗ-ЗА НИХ CI СТОЯЛ КРАСНЫМ МЕСЯЦ.
    # Модули опубликовали и не объявили; гейт отказал и был ПРАВ. Красная
    # галочка на публичном репозитории говорит входящему, что работа не
    # держится, — и ровно это она и говорила справедливо.
    "depth.py",          # глубина просадки: лучше ли ход после падения на u
    "loadcheck.py",      # почему 399 стратегий не загрузились ни разу
    "resolvable.py",     # хватает ли истории на заявление о барьерах
    # 21.09.2026: второй прибор — СТАТИЧЕСКИЙ, читает исходник без запуска.
    # Откалиброван против динамики freqtrade на этом же реестре: числа в
    # STATIC.md. Владелец файла — репозиторий APEX_P, здесь копия; копия
    # обязана совпадать побайтово, иначе наружу уходит непроверенное.
    "foreign_strategy_audit.py",  # статика: заглядывание · ручки · барьеры · случайность
    "calibrate.py",      # статика против динамики на ЭТОМ реестре: матрица + механизмы
